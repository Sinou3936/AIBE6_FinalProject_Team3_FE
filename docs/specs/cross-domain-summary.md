...
| `risk-analysis`     | 거의 완전 구현                  | **(2026-07-31 갱신)** Backend가 이미 전부 구현해뒀던 걸 확인하고 FE 연동 완료 — 매물 상세 카드(신호 미리보기 2개 + 보증금 안전성 미니 섹션) + 전용 페이지(`/properties/[id]/risk-analysis`, 신호 4종 전체 + 보증금 안전성 전체). **(2026-08-07 정정)** 선순위보증금 재계산 폼은 이미 연동 완료 상태였음(문서가 낡아있었을 뿐) — `risk-analysis-design.md` 참고. 매물 수정 시 위험 신호 자동 재계산(`RiskRecalculationService`, 내부 이벤트 리스너)도 FE 연동 불필요로 확인됨. 남은 건 150% 초과 전용 경고 문구 없음(Backend 필드 부재) + `/contract/result` 보증금 탭 미연동                      |
