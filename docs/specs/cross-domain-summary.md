...
| `checklist` | 거의 완전 구현 | **(2026-07-30 갱신)** 확인 필요 4개 — Backend의 정렬/삭제매물 재검증 해결에 이어, FE도 필수/일반 배지·Access Token 만료 재로그인·disclaimer 동적화·필수 항목 쉬운 설명(helperText)·체크리스트 완료 버튼을 직접 구현. 다만 helperText는 Backend 컬럼 대기 중(FE mock만), "최종 점검일"은 자리만 있고 데이터 없음, 특약사항 분석과의 여정 연결(propertyId 전달·결과 저장)은 브레인스토밍만 하고 미해결로 새로 3개가 늘어남(기존 risk-analysis 연계 자리 없음 포함 총 4개) |
...
Backend 문서의 같은 패턴이 FE에도 그대로 나타납니다.

- `property` 목록/상세 카드의 `checkSignalCount`/`jeonseRatio`가 실제 API 모드에서는 항상 `undefined`라 "준비 중" 배지로만 표시됨
- `checklist`의 소유권취득일 항목에 risk-analysis 연계를 표시할 자리 자체가 없음
- `contract-analysis`의 "보증금" 탭이 완전 정적 데이터(전세가율 82% 등 하드코딩)
- `home` 화면의 "확인 필요 신호" 섹션도 `checkSignalCount`가 항상 없어서 사실상 항상 빈 상태

넷 다 같은 원인(시세·위험도 데이터가 없어 연동할 게 없음)의 다른 얼굴이라는 점이 Backend 문서와 동일합니다. `market-data`부터 순서를 잡는 게 Backend·Frontend 양쪽 모두에 자연스러워 보입니다.
...
| property | 6개 |
| market-data | 4개 |
| checklist | 4개 (2026-07-30 갱신, 위 표 참고) |
