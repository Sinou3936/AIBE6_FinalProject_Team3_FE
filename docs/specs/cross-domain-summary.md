...
| `checklist` | 거의 완전 구현 | **(2026-07-29 갱신)** 확인 필요 1개로 축소 — Backend의 정렬/삭제매물 재검증 해결에 이어, FE도 필수/일반 배지 추가 + Access Token 만료 시 재로그인 유도(`isSessionInvalidErrorCode`)를 직접 구현·검증 완료. 403 메시지 구체화는 실사용 경로가 없어 보류. 남은 건 risk-analysis 연계 신호 자리 없음(다른 도메인 구현 대기) |
...
| property | 6개 |
| market-data | 4개 |
| checklist | 1개 (2026-07-29 갱신, 위 표 참고) |
