...
| `checklist`         | 거의 완전 구현                  | **(2026-08-13 갱신)** 확인 필요 2개 유지 — `GET /checklists` 페이지네이션 대응 완료(BE `PageResponse` 전환에 맞춰 FE `ChecklistOverviewPage` 도입, mock도 실 API와 동일 페이지 크기로 통일) + "내 체크리스트 목록" 카드 제목이 매물유형 문자열 대신 실제 매물명(`title`)을 쓰도록 BE/FE 함께 수정 완료. 남은 2개는 (1) 매물 접근권한/삭제 관련 에러 코드 구분을 화면 문구에 안 살리는 것(URL 직접 조작 시에만 발생, 의도적 보류) (2) 특약사항 분석과의 여정 연결(propertyId 전달·결과 저장, 브레인스토밍만 하고 미해결) — `checklist-design.md` 참고 |
| `user`              | 부분 구현                       | **(2026-08-12 갱신)** 이미지 업로드(S3 presign/confirm/reset)는 `feat/profile-s3-fe`가 `dev`에 머지되어 실제 배포됨 ✅. 회원 탈퇴 연결(`feat/withdraw-connect`)과 닉네임 형식 검증(`feat/user-nickname-format`)도 구현은 끝났지만, 둘 다 아직 origin에 push도 안 됐고 `dev` 머지도 안 된 별도 브랜치에만 있음 — 지금 `dev`/배포판 기준으로는 여전히 미구현 상태 |
...
**(2026-08-07 정정)** `contract-analysis`는 이미지 업로드(드래그앤드롭/파일선택/미리보기, JPG·PNG 형식 검증)가 실제로 구현됐습니다. **(2026-08-12 추가 정정)** `user`도 이제 해당됩니다 — 아래는 `property` 하나로 좁혀졌습니다.
...
| user              | 4개 (2026-08-12 갱신, 13개 중 9개 해결. 남은 건 "취업 여부" 요구사항 모호함 1건 + 위젯 재배치 미구현 1건 + property/checklist 세션 리다이렉트(스코프 밖) 1건 + 신규 추가된 욕설/금칙어 필터링 미구현 1건 — `user-design.md` 참고) |
| property          | 6개                               |
| market-data       | 4개                               |
...
1. **`useLogout.test.ts`(`frontend/app/lib/useLogout.test.ts`)에 로그아웃 실패(catch) 경로 테스트가 전혀 없다.** 이 파일은 테스트가 딱 1개(`logout() 호출 전에 먼저 resetAuthRefreshState()를 호출하고, 성공 후 한 번 더 호출한다`, L24-41)뿐이고, 성공 경로만 다룬다. 그런데 실제 구현(`frontend/app/lib/useLogout.ts:54-61`)은 `logoutOnce()`가 실패하면 `router.push('/login')`으로 이동시키지 않고 `setLogoutError('로그아웃하지 못했습니다...')`를 세팅하며 `isLoggingOut`을 다시 `false`로 되돌린다 — 주석(L34-36)에 "무조건 /login으로 보내면 사용자는 로그아웃된 줄 알지만 서버 세션은 그대로 남는다"는 명시적 설계 근거까지 적어둔, 의도적으로 지켜야 하는 동작이다. 하지만 이 동작을 지키는 테스트가 없어서, 누군가 실수로 catch 블록의 얼리리턴/상태복원을 지우고 성공 경로와 동일하게 `router.push('/login')`을 호출하도록 바꿔도 이 테스트 파일은 여전히 통과한다(원래 있던 성공 케이스 하나만 그대로 맞아떨어지므로). 실패 시 `logout`이 reject하도록 mock한 뒤 `push`가 호출되지 않고 `logoutError`가 채워지는지 확인하는 테스트가 빠져 있다.
2. **`SessionRecoverRetryButton.test.tsx`(`frontend/app/login/SessionRecoverRetryButton.test.tsx`)도 성공 경로(오픈 리다이렉트 방지)만 다루고, `getCurrentUser()` 실패 시 분기를 전혀 검증하지 않는다.** 실제 컴포넌트(`frontend/app/login/SessionRecoverRetryButton.tsx:28-36`)는 `catch` 블록에서 `isUnreachableError(error)`이면 `setRetrying(false)`로 되돌려 사용자가 버튼을 다시 누를 수 있게 하고, 그렇지 않으면(세션이 확실히 무효로 확인된 경우) 아무것도 하지 않는다(주석에 "이미 페이지 이동이 시작된 뒤라 여기 도달하지 않는다"는 전제가 적혀 있음). 이 두 분기 중 어느 쪽도 테스트가 없다 — `getCurrentUser`가 `isUnreachableError`로 판정되는 에러로 reject했을 때 버튼 라벨이 "다시 시도"로 돌아오는지(재시도 가능 상태로 복원되는지)를 검증하는 테스트가 없어서, 이 catch 분기가 깨져도(예: 조건을 반대로 바꾸거나 `setRetrying(false)`를 지워도) 잡아낼 수 없다. 이 컴포넌트 자체가 "크로스오리진 배포에서만 쓰는" 방어적 재시도 로직이라는 점(주석 L8-14)을 감안하면, 정확히 그 목적(재시도 가능 상태 유지)이 테스트로 보호되지 않는다는 게 아쉽다.
3. **`AdminUsersClient.test.tsx`(`frontend/app/(main)/admin/users/AdminUsersClient.test.tsx`)는 "정지" 액션의 성공/실패 경로를 전혀 다루지 않는다.** 4개 테스트 중 3개는 행 렌더링(본인 계정/타인/탈퇴유저) 검증이고, 나머지 1개(L79-102)는 "관리자 지정" 액션의 실패-후-모달-리셋만 다룬다 — `updateAdminUserStatus`(정지/정지해제)가 실제로 호출되는지, 실패 시 어떻게 되는지를 검증하는 테스트는 하나도 없다(컴포넌트 코드 L103 `updateAdminUserStatus` 참고, mock은 선언만 되어 있고 어떤 테스트에서도 `mockResolvedValueOnce`/호출 검증에 쓰이지 않는다). "다른 유저의 행에는 관리자 해제/정지 버튼을 정상적으로 보여준다" 테스트(L50-62)도 버튼이 "보이는지"만 확인하고 클릭 시 실제로 무슨 일이 일어나는지는 확인하지 않는다.
