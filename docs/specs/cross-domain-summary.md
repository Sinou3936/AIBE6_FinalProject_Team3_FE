...
| `checklist`         | 거의 완전 구현                  | **(2026-07-31 갱신)** 확인 필요 1개로 축소 — Backend가 helperText 컬럼과 "최종 점검일"(lastCheckedAt + 정렬)까지 마저 구현해서 FE 연동 완료, FE도 완료/미흡 버튼 취소(토글)·미확인 개수 안내·말풍선 UI를 추가로 구현. risk-analysis 연계 신호도 risk-analysis 도메인 연동으로 해소됨. **(2026-08-06 추가)** `GET /checklists`가 Backend에서 페이지네이션 응답(`PageResponse`)으로 바뀌면서 FE도 `ChecklistOverviewPage` 도입 + 이전/다음 페이지 UI로 대응 완료(매물 목록과 동일한 패턴, `checklist-design.md` 참고). 남은 건 특약사항 분석과의 여정 연결(propertyId 전달·결과 저장, 브레인스토밍만 하고 미해결) |
...
| `risk-analysis`     | 거의 완전 구현                  | **(2026-07-31 갱신)** Backend가 이미 전부 구현해뒀던 걸 확인하고 FE 연동 완료 — 매물 상세 카드(신호 미리보기 2개 + 보증금 안전성 미니 섹션) + 전용 페이지(`/properties/[id]/risk-analysis`, 신호 4종 전체 + 보증금 안전성 전체). **(2026-08-07 정정)** 선순위보증금 재계산 폼, 150% 초과 경고 문구 둘 다 이미 구현 완료 상태였음(문서가 낡아있었을 뿐, Backend 계약 변경 불필요) — `risk-analysis-design.md` 참고. 매물 수정 시 위험 신호 자동 재계산(`RiskRecalculationService`, 내부 이벤트 리스너)도 FE 연동 불필요로 확인됨. 남은 건 `/contract/result` 보증금 탭 미연동 + `property-detail.ts` 죽은 코드(`riskSummaries`) 정리뿐                      |
