...
| `checklist`         | 거의 완전 구현                   | **(2026-08-19 갱신)** `GET /checklists` 페이지네이션·매물명 표시 수정, 문항별 참고 이미지·`MULTIPLE_CHOICE`(선택지 응답) 지원 및 관리자용 CRUD 완료에 이어, 사용자 피드백을 반영한 UX 개선 라운드 진행: `/checklists` 목록에 진행률(%)·주의 항목 수 표시(홈/마이페이지/매물목록과 통일), 체크리스트 상세 뒤로가기를 `router.back()`으로 변경, 참고 사진 확대 모달에 이전/다음 이동 + 닫기(X) 버튼 추가, "필수 확인 누락 N개" 클릭 시 해당 항목으로 이동, 채광/수압/냉난방/층간소음/외부소음 5개 항목에 구체적 확인 방법 안내(`guideText`) + `양호/보통/미흡` 응답 방식 전환(+ "미흡" 선택 시 자동 주의 항목 판정, "미흡"만 다른 색상 표시)까지 전부 반영 — `checklist-design.md` 참고. "확인 필요" 항목은 3개(에러 코드 구분 미반영, 특약사항 분석 여정 연결, mock의 code 기반 자동판정 미반영)로 늘어남(신규 발견 1건 포함) |
| `user`              | 거의 완전 구현                   | **(2026-08-14 갱신)** 이미지 업로드(`feat/profile-s3-fe`), 회원 탈퇴 연결(`feat/withdraw-connect`), 닉네임 형식 검증(`feat/user-nickname-format`, PR #135) 전부 `dev`에 머지되어 실제 배포됨 ✅. 팀원 전수조사로 발견된 버그 3건(프로필 저장 시 이미지만 반영되고 필드 저장은 실패하는 문제, 홈 화면 체크리스트 위젯 `Promise.all` 전체 실패, 홈 화면 "새로고침" CTA 무효)도 `fix/user-domain-complete`(PR #144)로 수정해 `dev`에 머지됨. **(2026-08-19 갱신)** 위젯 재배치 방향은 결정됨 — 진행 스텝퍼 방향을 시도했으나 체크리스트 반복성/특약사항 분석 신호 부재 문제로 폐기하고, 온보딩 안내 모달(`OnboardingIntroModal`)로 대체 구현 완료(`user-design.md` 참고). **(2026-08-21 갱신)** 외부 피드백으로 `regions_nested.ts`(법정동코드 데이터)의 대량 중복 항목(76%)을 발견·제거하고, 시/도만 정적으로 남기고 시·군·구/읍·면·동은 신규 Route Handler(`app/api/regions/*`)로 온디맨드 서빙하도록 구조 자체를 바꿔 마이페이지 진입 속도 이슈에 대응함 — 아래 "공통/인프라 코드 전수조사 결과" 4번, `user-design.md` 이슈 14번 참고. 같은 날 "페이지 이동 시 화면 깜빡임(특히 홈/마이페이지)" 피드백도 확인 — `home/page.tsx`/`mypage/page.tsx`가 프로필/매물/체크리스트/활동내역을 순차 `await`로 하나씩 불러오던 걸 `Promise.allSettled`로 병렬화하고, 백엔드에 없는 `/users/me/activity-history` 호출은 `ENABLE_ANALYSIS_HISTORY` 플래그로 아예 건너뛰도록 수정(`user-design.md` 이슈 15번). 마이페이지 "등록 매물" 목록도 `PropertyListSection.tsx`(신규)로 분리해 5개씩 페이지네이션 적용(`ContractHistorySection.tsx`와 동일 패턴, `user-design.md` 이슈 16번) — Property 도메인이 `MarketComparisonService.getCachedOnly()`로 목록 조회 시 외부 API 호출을 차단해둔 덕에(백엔드 이슈 #258) 요약 합계용 추가 조회(size=100)를 그대로 유지해도 안전함을 확인하고 진행. "취업 여부" 요구사항 모호함도 위젯 재배치 전제가 폐기되면서 다루지 않기로 결정됨(`user-design.md` 이슈 2번). **(2026-08-24 갱신)** 플래그로 호출만 막아뒀던 `/users/me/activity-history` 관련 코드(`activityHistory.ts`/매퍼/레포지토리/목데이터, `ActivityHistoryItem(Dto)` 타입, `config/features.ts`)를 전부 제거하고, 팀원이 완성한 계약분석 이력 API(`getMyContractHistory()`)로 대체함 — 홈 화면 "확인 필요" 알림도 `type` 문자열 대신 구조화된 `riskCount > 0`로 정확히 판정하도록 함께 고쳐짐(`user-design.md` 이슈 17번). **(2026-08-24 추가 갱신)** "취업 여부는 다루지 않기로" 결정에서 한 걸음 더 나가, 관련 필드였던 `currentStage`(자취 처음/경험 있음) 자체를 백엔드 엔티티·DB 컬럼까지 포함해 완전히 제거함 — `UserProfile`/DTO에서 필드 삭제, `ProfileClient.tsx`의 선택 UI 삭제, `MyPageClient.tsx`의 등록 안내 문구는 이미 있던 `hasRegisteredProfile()` 기반으로 대체, `getPriorityAction()`의 "자취 처음" 문구 분기도 삭제해 매물 미등록 시 항상 같은 문구를 보여줌(`user-design.md` 이슈 2번/4번). 남은 건 property/checklist 세션 리다이렉트(스코프 밖) 1건뿐                                                                                                             |
...
| `risk-analysis`     | 거의 완전 구현                   | **(2026-08-21 갱신)** 위험 신호 4종 + 보증금 안전성(전세가율) + 선순위보증금 재계산까지 연동 완료, `contract-analysis` "보증금" 탭 연동도 이미 되어 있었음(문서만 안 갱신됨). 전수조사 재검증으로 버그 2건 + 코드 품질 1건(전세가율 색상 판정 기준 하드코딩) 수정에 이어, 사용자 피드백으로 위험 신호에 구체적 후속 행동 안내(`recommendedActions`, 체크리스트 형태 불릿 목록)를 추가 — `PRICE_ANOMALY` 신호에 "동일 지역 실거래가 확인/등기부등본 확인/가격 비교/중개사 문의/추가비용 확인" 5개 항목을 보여줌. 남은 건 `property-detail.ts`의 죽은 코드(`riskSummaries`) 정리 정도 — `risk-analysis-design.md` 참고                                                                                                                                                                                                         |
...
| user              | 1개 (2026-08-24 갱신, 17개 중 16개 해결/결정 완료 — "취업 여부"도 콘텐츠 차별화 전제(위젯 재배치)가 폐기되면서 다루지 않기로 결정됨. 홈/마이페이지 데이터 조회 순차 실행으로 인한 로딩 지연(화면 깜빡임 피드백)을 병렬화로 해결하고, 마이페이지 "등록 매물" 목록에 페이지네이션 적용, `/users/me/activity-history`를 계약분석 이력 API로 전면 대체까지 완료. 남은 건 property/checklist 세션 리다이렉트(스코프 밖) 1건뿐 — `user-design.md` 참고) |
| property          | 6개                                                                                                                                                                                                                                                                                              |
| market-data       | 4개                                                                                                                                                                                                                                                                                              |
| checklist         | 3개 (2026-08-19 갱신, 위 표 참고)                                                                                                                                                                                                                                                                |
| contract-analysis | 6개 (2026-08-07 갱신, 위 표 참고)                                                                                                                                                                                                                                                                |
| risk-analysis     | 1개 (2026-08-21 갱신, 위 표 참고)                                                                                                                                                                                                                                                                |
...
4. **(2026-08-21 외부 피드백으로 뒤늦게 발견 — 이 전수조사 범위(`app/data` 포함)에서도 실제로는 놓쳤던 문제)** `app/data/regions_nested.ts`(법정동코드 원본, 당시 21,203줄)의 `eupmyeondong` 배열이 읍/면 하위 리(里) 단위 세부 코드까지 행으로 포함하고 있었는데, 그 리 행들의 `name`이 리 이름이 아니라 상위 읍/면 이름을 그대로 복사한 값이라 전체 19,841개 항목 중 15,097개(76%)가 코드만 다른 순수 중복이었다(예: 강릉시 "강동면"이 11번 반복). `app/data/*.ts` 나머지 파일은 전부 5~82줄 수준이라(`wc -l` 확인) `regions_nested.ts` 하나만 압도적으로 큰 이상치인데도, "파일이 유독 크다"는 사실 자체가 이 전수조사에서 신호로 잡히지 않았다 — 줄 수가 아니라 실제 값을 몇 군데 열어봐야 중복이 드러나는 유형의 문제라, 향후 비슷한 전수조사에서는 `app/data` 파일들의 줄 수 편차 자체를 이상 신호로 먼저 훑어보는 게 안전해 보인다. `user-design.md` 이슈 14번에서 시군구별 중복 제거(21,203줄→6,108줄, 약 74% 감소)로 1차 대응한 뒤, 시/도만 정적으로 남기고(`app/data/sido.ts`) 시·군·구/읍·면·동은 신규 Route Handler(`app/api/regions/sigungu`, `app/api/regions/eupmyeondong`)로 온디맨드 서빙하는 구조로 재구성해 `regions_nested.ts` 자체가 클라이언트 번들에서 완전히 빠지도록 해결함. 이 프로젝트의 Route Handler는 그동안 `app/auth/session-recover/route.ts`(인증 쿠키 조작 전용) 하나뿐이었는데, 이번에 "로컬 정적 데이터를 온디맨드 JSON으로 서빙"하는 새 용도로 처음 쓰였다 — 지금은 `app/data/*` 중 이만큼 큰 파일이 더 없어 당장 다른 적용 대상은 없지만, 나중에 비슷하게 비대한 정적 데이터가 생기면 참고할 선례로 남겨둔다.
