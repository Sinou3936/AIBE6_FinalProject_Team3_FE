...
| 도메인 | 상태 | 비고 |
| --- | --- | --- |
| `auth` | 거의 완전 구현 | **(2026-07-28 갱신)** 확인 필요 1개로 축소 — matcher 누락/비밀번호 정책 중복 해결, 에러 메시지 pass-through는 의도된 설계로 재분류됨. 남은 1개(클라이언트 사이드 자동 재발급 불가)는 브라우저-side refresh-retry 흐름 미구현 상태로 보류 |
