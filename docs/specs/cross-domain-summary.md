...
| `user`              | 대부분 구현                     | **(2026-08-14 갱신)** 이미지 업로드(`feat/profile-s3-fe`), 회원 탈퇴 연결(`feat/withdraw-connect`), 닉네임 형식 검증(`feat/user-nickname-format`, PR #135) 전부 `dev`에 머지되어 실제 배포됨 ✅. 팀원 전수조사로 발견된 버그 3건(프로필 저장 시 이미지만 반영되고 필드 저장은 실패하는 문제, 홈 화면 체크리스트 위젯 `Promise.all` 전체 실패, 홈 화면 "새로고침" CTA 무효)도 `fix/user-domain-complete`(PR #144)로 수정해 `dev`에 머지됨. 남은 건 "취업 여부" 요구사항 모호함/위젯 재배치 방향 미정처럼 방향 결정이 필요한 항목들뿐 — `user-design.md` 참고 |
...
| user              | 3개 (2026-08-14 갱신, 13개 중 10개 해결/결정 완료 — 욕설/금칙어 필터링은 구현하지 않기로 결정되어 해결 항목으로 전환됨. 남은 건 "취업 여부" 요구사항 모호함 1건 + 위젯 재배치 방향 미정 1건 + property/checklist 세션 리다이렉트(스코프 밖) 1건 — `user-design.md` 참고) |
| property          | 6개                               |
| market-data       | 4개                               |
