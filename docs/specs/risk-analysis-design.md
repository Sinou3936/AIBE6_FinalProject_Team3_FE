...
| PRICE_ANOMALY(시세 이상 저가) 신호와 전세가율 판정 모순 방지 | ✅ **(2026-08-19 해결, #189, BE #241)** 보증금이 시세보다 비정상적으로 싸서 PRICE_ANOMALY 신호가 뜬 매물이, 그 낮은 보증금 때문에 전세가율은 오히려 "안전"으로 나오는 모순을 실사용 화면에서 발견(BE risk-analysis-design.md 남은 이슈 13번 참고). Backend `DepositSafetyCheckResponse.priceAnomalyWarning`을 받아 `recentOwnershipChangeWarning`과 동일한 패턴으로 "이 매물은 가격 자체가 시세보다 비정상적으로 낮아 의심돼요. 전세가율이 안전해 보여도 가격의 신뢰도를 먼저 확인하세요" 경고 배너를 병기 |
...
- **위험 신호에 구체적 후속 행동 안내(`recommendedActions`) 추가**(2026-08-21, 사용자 피드백) — 기존엔 신호별 `description`이 "왜 이렇게 저렴한지 확인해보세요"처럼 뭉뚱그린 한 줄이라, 실제로 뭘 확인해야 하는지 알기 어렵다는 피드백이 있었음. 두 방향(A: 기존 `description` 문자열에 그대로 녹이기 — BE 단독으로 끝나지만 문장이 길어지고 체크리스트 형태로 보여주기 어려움 / B: 구조화된 필드 추가 — API 스키마는 바뀌지만 항목별로 낱개 렌더링 가능)을 검토했고, "체크리스트 형태로 낱개 액션을 보여줘야 실제 행동으로 이어진다"는 피드백 의도에 더 맞는 B안으로 결정(FE 작업량도 기존 신호 카드 패턴에 목록 하나 추가하는 수준이라 우려했던 것보다 작았음). Backend가 `RiskSignalResponse`에 `recommendedActions: List<String>`을 신호 타입 공통 필드로 추가(현재는 `PRICE_ANOMALY`만 실제 값을 채움 — "동일 지역 최근 실거래가 확인" · "등기부등본 확인" · "주변 동일 면적 매물 가격 비교" · "중개사에게 가격이 낮은 이유 확인" · "옵션/관리비 등 추가 비용 확인" 5개(`RiskSignalResponse.PRICE_ANOMALY_ACTIONS`와 문구 정확히 일치), 다른 신호 타입은 항상 빈 배열). FE도 `RiskSignalDto`/`RiskSignal`에 동일 필드를 추가하고 `RiskAnalysisClient.tsx`의 신호 카드에 불릿 목록으로 렌더링. `PropertyDetailClient.tsx`의 "확인 필요 신호" 요약 카드(최대 2개 미리보기 + "자세히 보기" 링크로 전용 페이지 연결)에는 의도적으로 추가하지 않음 — 요약은 간결하게 유지하고, 상세 액션은 이미 있는 "자세히 보기" 흐름을 통해 전용 페이지에서 보여주는 쪽이 기존 정보 구조와 맞다고 판단
