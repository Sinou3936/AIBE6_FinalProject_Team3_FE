...
| 실거래가 비교 근거(원본 표본) 노출 | ✅ **(2026-08-21 추가, #264/#197, 5차 멘토링 피드백 7-2)** 상세 페이지 "실거래가 비교" 카드에 기준 시세 산출에 쓰인 국토교통부 실거래 원본 표본 목록("실거래 내역")을 추가 — 건물명/주소, 계약일, 보증금, 면적(평 병기)을 최신 계약일순으로 나열. BE `MarketComparisonResponse.samples`(`MarketTransactionSampleResponse[]`)를 그대로 매핑(`mapMarketComparisonSampleDto`). 국토부 실거래가는 이미 법정 공개 데이터라 개별 건 노출에 별도 프라이버시 이슈 없음 |
