...
| 건물명(아파트/오피스텔명 등)으로 검색 | ✅ **(2026-08-21 추가/정정, #264/#197, 5차 멘토링 피드백 6-3)** 처음엔 "상세 필터" 패널에 별도 건물명 입력란 + `title` 쿼리파라미터로 구현했었는데, 메인 검색창은 하나뿐이라 사용자가 그 칸에 주소든 건물명이든 입력하면 찾아지길 기대하는 UX였음 — 별도 입력란은 제거하고, 상단 메인 검색창(`region` 상태/쿼리파라미터)이 주소든 건물명이든 매칭되도록 통합함(BE `PropertyRepository.search()`가 `region` 값으로 주소·건물명을 OR 매칭). placeholder도 "지역(도로명/지번주소) 또는 건물명으로 검색하세요"로 수정 |
| 실거래가 비교 근거(원본 표본) 노출 | ✅ **(2026-08-21 추가/정정, #264/#197, 5차 멘토링 피드백 7-2)** 상세 페이지 "실거래가 비교" 카드에 기준 시세 산출에 쓰인 국토교통부 실거래 원본 표본 목록("인근 실거래 내역")을 추가 — 건물명/주소, 계약일, 보증금, 면적(평 병기)을 최신 계약일순으로 나열. BE `MarketComparisonResponse.samples`(`MarketTransactionSampleResponse[]`)를 그대로 매핑(`mapMarketComparisonSampleDto`). 표본이 많으면 다 보여주기 부담스럽다는 피드백을 받아 BE가 대표 최대 5건(최고가/최저가/최근순)만 내려주도록 제한함(`market-data-design.md` 참고) — FE는 `sampleCount`(총 표본 수)가 `samples.length`보다 크면 "총 N건 중 대표 5건" 안내 문구를 같이 보여준다. 목록 자체는 최신순 정렬이라 최고가/최저가 표본이 중간에 섞여 있으면 왜 포함됐는지 알기 어려워, BE가 내려주는 `priceHighlight`(`HIGHEST`/`LOWEST`/null)를 각 항목에 작은 배지("최고가"/"최저가")로 표시한다. 국토부 실거래가는 이미 법정 공개 데이터라 개별 건 노출에 별도 프라이버시 이슈 없음 |
| 검색/필터 시 화면 전환 없이 결과만 갱신 | ✅ **(2026-08-24 개선, #204, 5차 멘토링 피드백 5번)** `page.tsx`가 검색/필터/페이지 변경으로 재조회할 때마다 `PropertiesClient` 자체를 언마운트하고 전체화면 스피너로 교체하던 구조라, 실제로는 client fetch인데도 페이지 전체가 리로드되는 것처럼 보였음 — 최초 진입(`hasLoadedOnce=false`)에만 전체화면 스피너를 쓰고, 이후 재조회는 `PropertiesClient`를 계속 마운트한 채 `loading` prop만 내려서 결과(카드) 영역만 반투명 처리 + 스피너 오버레이로 표시. 검색창/필터 패널/이미 있던 결과는 로딩 중에도 계속 보이고 조작 가능(단, 결과 영역은 `pointer-events-none`으로 잠금) |
...
| **주소 변경 시 재정규화·좌표 재계산** | ❌ **주소 자체를 수정할 수 없음.** 화면에 "주소와 매물·거래 유형은 등록 시 확정된 값이라 수정할 수 없어요. 변경이 필요하면 매물을 새로 등록해주세요"라고 명시돼 있고, `UpdatePropertyRequestDto`에도 주소/유형 필드 자체가 없음. 단, 동/호수 등 상세주소(`detailAddress`)는 예외적으로 등록/수정 폼 양쪽에서 입력·수정 가능(5차 멘토링 피드백 3번, #202/#286) — 같은 건물 안 여러 매물을 구분하기 위한 순수 표시용 텍스트라 지오코딩 대상이 아니고, 목록/상세 화면 주소 뒤에 이어붙여 노출된다 |
