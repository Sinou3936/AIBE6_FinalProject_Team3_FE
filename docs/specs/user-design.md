...
| 요구사항 | 실제 구현 |
| --- | --- |
| 탈퇴 요청 | ⚠️ **프론트 버튼이 백엔드 API에 연결되지 않음.** `MyPageClient.tsx`에 "회원 탈퇴" 버튼은 있지만 `handleWithdrawClick`이 빈 함수 + `// TODO: 회원 탈퇴 확인 모달 연동 (백엔드 탈퇴 API 확정 후 진행)` 주석뿐. 클릭해도 아무 일도 안 일어남. ~~(백엔드 탈퇴 API 확정 후 진행)~~ — **(2026-08-12 정정)** 주석의 전제와 달리 **백엔드 `DELETE /users/me`(`UserController.java:77-82`)는 이미 완성돼 있다**(`backend/docs/specs/user-design.md` "회원 탈퇴" 절 참고, 본인확인/상태변경/부분적 개인정보 익명화까지 구현됨). 즉 백엔드가 안 나와서가 아니라 **프론트 연결 작업이 안 된 상태**다 |
| 본인 여부 확인 / 상태 변경 / 개인정보 삭제·익명화 / 클라이언트 인증 정보 제거 | ⚠️ 프론트에서 호출하는 함수가 없어 실질적으로 미구현이지만(`app/services/user.ts`에 탈퇴 관련 함수 없음), **API 자체는 이미 존재**한다 — 위 행 참고 |
| 성공/실패 결과 처리 | ❌ 미구현(프론트 연결 자체가 안 됨) |
...
1. **회원 탈퇴 프론트 연결이 미구현 상태** — 버튼은 있지만 클릭해도 아무 동작이 없는 TODO 스텁. ~~백엔드 탈퇴 API가 나오면 그때 이어서 구현 필요(주석에도 이미 명시됨)~~ — **(2026-08-12 정정)** 백엔드 `DELETE /users/me`는 이미 구현되어 있음(`backend/docs/specs/user-design.md` 참고) — 남은 건 프론트 확인 모달 + 서비스 함수 연결뿐. **(2026-07-29)** 팀 논의 결과 property/checklist 등 타 도메인 작업이 더 진행된 뒤 다시 검토하기로 하고 보류(이 보류 결정 당시엔 백엔드 API 완성 여부가 불확실했을 수 있음 — 지금은 API가 있으므로 재검토 시 프론트 작업만 남았다는 점을 반영)
...
1. **(신규 발견)** `ProfileClient.tsx`의 `handleSubmit`(`app/(main)/mypage/profile/ProfileClient.tsx:237-260`)은 이미지 처리(`uploadProfileImage`/`resetProfileImage`)와 나머지 필드 저장(`registerProfile`/`updateMyProfile`)을 두 번의 독립된 API 호출로 순차 실행하며, 첫 번째가 성공한 뒤 두 번째가 실패해도 앞의 변경을 되돌리는 로직이 없다. 이미지 업로드/초기화가 서버(및 S3)에 이미 반영된 뒤 `registerProfile`/`updateMyProfile`이 닉네임 경쟁(409)이나 네트워크 오류로 실패하면, 화면에는 `saveError` 문구만 뜨고 사용자는 "저장이 통째로 실패했다"고 여기지만 실제로는 프로필 사진(또는 기본 이미지로의 초기화)만 이미 서버에 반영된 채로 남는다. 특히 `mode === 'register'`(온보딩) 상태에서 이런 실패가 나면, 아직 `UserPreference`가 생성되지 않아 온보딩이 끝나지 않은 계정에 사진만 먼저 바뀌어 있는 상태가 된다. 두 호출을 하나의 트랜잭션으로 묶을 수는 없지만(백엔드 API가 분리되어 있어), 최소한 이미지 처리 실패/성공과 무관하게 나머지 필드 저장이 실패했을 때 사용자에게 "사진은 이미 반영되었다"는 사실을 알려주거나, 순서를 바꿔 필드 저장을 먼저 시도하는 것을 고려할 수 있다.
