...
export function PasswordUpdateFormClient({ hasPassword, passwordPolicy }: PasswordUpdateFormClientProps) {
  ...
  const handleSubmit = async (event: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      // 'unreachable'(네트워크 오류 등, requestJson()이 강제 로그아웃하지 않고 원래 에러를 그대로
      // 던지는 경우)에 대한 fallback UX뿐이다 — 그래도 세션이 유효하지 않을 가능성이 있으니 폼
      // 에러 대신 재로그인 화면으로 보낸다. 화면 문구는 사유별로 안 나누지만(docs/specs/auth-design.md
      // 참고) "재로그인이 필요한가" 판단은 네 코드를 전부 인식해야 한다.
