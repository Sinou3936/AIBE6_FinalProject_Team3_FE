...
export function computeHomeSummaryCounts(
  ...
  return {
    ...
    signalsToCheckCount: properties.reduce((sum, property) => sum + property.checkSignalCount, 0),
    activeChecklistCount: properties.filter((property) => property.checklist > 0 && property.checklist < 100).length,
